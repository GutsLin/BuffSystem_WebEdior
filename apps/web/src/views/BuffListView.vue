...
<template>
  <section class="page-container">
    <PageHeader eyebrow="Modifier Library" title="Buff 配置库" description="管理增益、减益、被动与光环模板。">
      <template #actions>
        <van-button type="primary" icon="plus" @click="router.push('/buffs/new')">新建 Buff</van-button>
      </template>
    </PageHeader>

    <div class="toolbar-card">
      <van-search v-model="query" placeholder="搜索名称、Key 或标签" shape="round" />
    ...
    <div v-else-if="filtered.length" class="buff-grid">
      <article v-for="buff in filtered" :key="buff.id" class="buff-card" @click="router.push(`/buffs/${buff.id}`)">
        <div class="buff-card-top">
          <span class="kind-badge" :class="buff.modifierKind.toLowerCase()">{{ buff.modifierKind }}</span>
          <span v-if="buff.isAura" class="soft-badge">Aura</span>
          <span v-if="buff.isPassive" class="soft-badge">Passive</span>
        ...
        <div class="buff-metrics">
          <span><b>{{ buff.duration < 0 ? '永久' : `${buff.duration}s` }}</b>持续</span>
    ...
    <van-empty v-else description="没有符合条件的 Buff">
